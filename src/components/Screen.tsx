...
// The chalkboard texture (a sage board streaked with freshly-erased chalk),
...
interface ScreenProps {
  ...
  bottomOffset?: number;
  /** Draw the chalkboard background + wooden frame. Default true. */
  board?: boolean;
 ...
 * the board (and the faint wooden frame) cover the whole screen, and renders an
 * in-screen header instead of the navigation header so nothing flat bands across
 * the top. Content sits in normal flow below the header, so scrolling lists never
 * bleed up through it. Text drawn straight on the board uses `colors.chalk`.
 *
...
export function Screen({
  ...
  return (
    <View style={styles.root}>
      ...
      {board ? <View pointerEvents="none" style={styles.frame} /> : null}
    </View>
  );
}
...
const styles = StyleSheet.create({
  ...
  pressed: { opacity: 0.6 },
  // The classroom chalkboard's wooden frame, hugging the screen edges.
  frame: {
    position: 'absolute',
    top: 0,
    left: 0,
